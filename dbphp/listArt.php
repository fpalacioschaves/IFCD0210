...
// Ahora extraemos filas individuales moviendo el puntero con mysqli_data_seek()
// y leyendo el objeto mysqli_fetch_assoc() ['NombreCampo']
for( $j = 0 ; $j < $rows ; ++$j)
{
	$result->data_seek($j);
	echo 'CodArt:   '.htmlspecialchars($result->fetch_assoc() ['CodArt']) . '<br>';
	
	$result->data_seek($j);
	echo 'DesArt:   '.htmlspecialchars($result->fetch_assoc() ['DesArt']) . '<br>';	
	
	$result->data_seek($j);
	echo 'PVPArt:   '.htmlspecialchars($result->fetch_assoc() ['PVPArt']) . '<br>';	
